...
export default function Register () {
  ...
  async function registerForm (values) {
    try {
      ...
      let { data } = await axios.post(
        createApiUrl('/api/auth/register'),
        values,
  ...
  let validationSchema = yup.object().shape({
    ...
    phone: yup
      .string()
      .matches(/^01(0|1|2|5)[0-9]{8}$/, 'Invalid phone number')
  ...
  const roleCards = [
    {
      ...
      alertBg: 'bg-[#f0fdf9]',
      alertBorder: 'border-[#46CEAC]',
      alertIcon: 'fa-heart',
      alertIconColor: 'text-[#0F6E56]',
      alertText: 'Signing up as a Pet Owner — access appointments & pet records',
      alertTextColor: 'text-[#0F6E56]',
      btnClass: 'bg-[#46CEAC] hover:bg-[#3ab89b]',
      focusRing: 'focus:ring-[#46CEAC]/30 focus:border-[#46CEAC]',
    },
    {
      key: 'doctor',
      icon: '🩺',
      ...
      alertBg: 'bg-blue-50',
      alertBorder: 'border-blue-300',
      alertIcon: 'fa-stethoscope',
      alertIconColor: 'text-blue-500',
      alertText: 'Signing up as a Veterinary Doctor — additional verification required',
      alertTextColor: 'text-blue-600',
      btnClass: 'bg-[#1a4fa0] hover:bg-[#163e82]',
  ...
  const formFields = [
    { name: 'name',     type: 'text',     placeholder: 'Enter your full name', label: 'Full Name' },
    ...
    { name: 'phone',    type: 'tel',      placeholder: '01XXXXXXXXX',          label: 'Phone number' },
  ]
    ...
    <section className='flex md:flex-row flex-col min-h-screen bg-[#F0F4FB] font-[Plus_Jakarta_Sans,sans-serif]'>
      ...
      <div className='relative left overflow-hidden bg-[#1a4fa0] px-8 py-10 md:w-[42%] w-full flex flex-col'>
        ...
        <p className='relative z-10 text-white/55 text-sm leading-relaxed mb-8'>
          Create your account and start managing appointments, records, and connect with our expert veterinary team.
        </p>
      ...
      <div className='right flex-1 bg-[#F0F4FB] px-8 py-10 overflow-y-auto'>
        ...
        {/* Role label */}
        <p className='text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-3'>I am a ...</p>
        ...
        <div className='grid grid-cols-2 gap-3 mb-6'>
          {roleCards.map(r => (
            <button
              ...
              className={`flex flex-col items-center bg-white border-[1.5px] rounded-2xl px-4 py-4 cursor-pointer transition-all duration-150
                ${role === r.key
                  ? `${r.activeClass} shadow-sm -translate-y-[2px]`
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                }`}
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-2 text-xl ${r.iconBg}`}>
                {r.icon}
              </div>
        ...
        {role && activeCard && (
          <div className='bg-white border border-gray-200 rounded-2xl p-5 shadow-sm'>
            ...
            <div className={`flex items-center gap-2 mb-4 pb-4 border-b border-gray-100`}>
              <i className={`fa-solid ${activeCard.alertIcon} ${activeCard.alertIconColor} text-base`} />
              <p className={`text-sm font-semibold ${activeCard.alertTextColor}`}>
                {activeCard.alertText}
              </p>
            </div>
            ...
            <form onSubmit={formik.handleSubmit} className='flex flex-col gap-3'>
                ...
                <div key={name} className='flex flex-col gap-1'>
                  ...
                  <input
                    ...
                    className={`bg-gray-50 px-3 py-2.5 border rounded-xl text-sm text-gray-800 outline-none transition
                      focus:ring-2 ${activeCard.focusRing}
              ...
              <button
                ...
                className={`flex justify-center items-center gap-2 ${activeCard.btnClass} active:scale-[0.98] disabled:opacity-60 mt-1 py-2.5 rounded-xl font-semibold text-white text-sm transition-all duration-150`}
              >
                {loadingSpinner
                  ? <i className='fa-solid fa-spinner fa-spin' />
                  : <>
                      <i className='fa-solid fa-user-plus text-sm' />
                      Create account as <span className='capitalize'>{role}</span>
                    </>
                }
              </button>
              ...
              <p className='text-center text-xs text-gray-500'>
                Have an account?{' '}
                <Link to='/Login' className='text-[#46CEAC] hover:text-[#3ab89b] font-semibold'>
                  Log in
                </Link>
              </p>
